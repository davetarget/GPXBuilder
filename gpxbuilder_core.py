...
def _add_gsak_style_header(gpx_root):
    """Ajoute au GPX de sortie les éléments d'en-tête que GSAK place lui-même
    entre <gpx> et le premier <wpt> dans ses propres exports (desc, author,
    email, time, keywords, bounds). GSAK semble s'appuyer sur la mention
    « (HasChildren) » présente dans ce <desc> pour savoir qu'il doit
    résoudre, à l'import, les liens gsak:wptExtension/Parent entre les
    waypoints additionnels et leur cache parente. Sans cet en-tête (absent
    du GPX minimal généré par ce script), GSAK peut importer tous les <wpt>
    à plat, sans les rattacher entre eux, même si le contenu de chaque <wpt>
    est par ailleurs parfaitement correct.
    """
    lats, lons = [], []
    for wpt in gpx_root:
        localname = wpt.tag.split("}")[-1] if "}" in wpt.tag else wpt.tag
        if localname != "wpt":
            continue
        try:
            lats.append(float(wpt.get("lat")))
            lons.append(float(wpt.get("lon")))
        except (TypeError, ValueError):
            continue

    header_specs = [
        ("desc", "Geocache file generated by GSAK (HasChildren)"),
        ("author", "GSAK"),
        ("email", "support@gsak.net"),
        ("time", datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")),
        ("keywords", "cache, geocache, groundspeak"),
    ]
    for i, (tag, text) in enumerate(header_specs):
        el = ET.Element(f"{{{GPX_NS}}}{tag}")
        el.text = text
        gpx_root.insert(i, el)

    if lats and lons:
        bounds_el = ET.Element(
            f"{{{GPX_NS}}}bounds",
            {
                "minlat": f"{min(lats)}",
                "minlon": f"{min(lons)}",
        ...
        gpx_root.insert(len(header_specs), bounds_el)
